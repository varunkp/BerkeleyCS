...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # and noisyDistance is None
          ...
          # handle everything related to putting it in a jail cell
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        noisyDistance = observation #e
        emissionModel = busters.getObservationDistribution(noisyDistance) #w(x) = P(e|x)
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        ...
        for pos in self.legalPositions:
            ...
            new_beliefs[pos] = beliefs[pos] * emissionModel[trueDistance]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        ghost_length = len(ghosts)

        for i in range(self.numParticles):
            j = i % ghost_length
            particles_list.append(tuple(ghosts[j]))
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            ...
            new_beliefs[tuple(particle)] += weight
        # new_beliefs.normalize()
        self.beliefs = new_beliefs
            ...
            for i in range(0, self.numParticles):
                ...
                for j in range(0, self.numGhosts):
                    if noisyDistances[j] == None:
                        new_particle.append(tuple(self.getJailPosition(j)))
                    else:
                        new_particle.append(util.sample(new_beliefs)[j])
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                curr_agent = self.ghostAgents[i]
                curr_state = setGhostPositions(gameState, newParticle)
                newPosDist = getPositionDistributionForGhost(curr_state, i, curr_agent)
